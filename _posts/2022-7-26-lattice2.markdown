...
**定义 1**: Short Integer Solution _problem $\mathbf{SIS}$, 输入一个矩阵 $\mathbf{A} \in \mathbb{Z}^{n\times m}\_{q}$，要求找到一个非零向量 $\mathbf{x}\in X$ such that $\mathbf{A x}=0 \quad(\bmod\enspace q)$_。
