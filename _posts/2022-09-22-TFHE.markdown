...
TFHE 通过将(负的) $b-\sum_{i=0}^{n-1} a_i s_i = \Delta m + e$ 的计算放在单项式 $X$ 的指数里面，然后用它对查找表(LUT)进行旋转，计算解密的第二步。从第二步开始。

## Step 2

如果想将 $\Delta m + e$ 处理为 $m$，需要将这两者联系起来。$\Delta m + e\in\mathbb{Z}\_q,m\in\mathbb{p},p<q$，所以我们需要重复 $m$ 做成一个 LUT，将所有 $\Delta m + e$ 都取整为 $m$。冗余的框框叫做 mega-cases。
