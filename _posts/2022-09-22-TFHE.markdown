...
大常数乘上密文需要先将大常数分解，然后在和 GLev 密文进行重组得到数乘结果，GLWE 密文由一连串的大常数组成，结合一下：

跟密钥更换类似，对于两个密文，拎一个出来当成要被分解的 GLWE 密文，另外一个是一串 GLev 密文。和密钥更换的不同点是，密钥交换只把 mask 拎出来当成系数，这次是 mask 和 body。

GGSW 正好满足要求(~~一连串的 GLev 密文~~)。所以输入一个 GLWE，一个 GGSW，输出一个 GLWE 密文加密了两个消息的乘积。

GLWE 密文加密了消息 $M\_1\in\mathcal{R}\_q$，密钥是 $\vec{S} = (S\_0, \ldots, S\_{k-1}) \in \mathcal{R}^k$：

$$C = (A_0, \ldots, A_{k-1}, B) \in GLWE_{\vec{S}, \sigma}(\Delta M_1) \subseteq \mathcal{R}_q^{k+1}$$

GGSW 密文加密了消息 $M\_2\in\mathcal{R}\_q$，密钥是 $\vec{S} = (S\_0, \ldots, S\_{k-1}) \in \mathcal{R}^k$：
