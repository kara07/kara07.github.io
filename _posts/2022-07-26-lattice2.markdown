...
**定义 5** *一个函数家族 $\mathcal{F}=\left\\{f\_{k}: X \rightarrow Y\right\\}$ 是抗碰撞的如果其对于任意PPT算法 $\mathcal{A}$，*
...
**定义 6** *一个函数家族 $\mathcal{F}=\left\\{f\_{k}: X \rightarrow Y\right\\}$ 是关于(可高效采样的)输入分布 $X$ 单向的如果其对于任意PPT算法 $\mathcal{A}$，*
...
**定义 7** *一个函数家族 $\mathcal{F}=\\{f\_{k}: X \rightarrow Y\\}$ 是一个在输入分布 $X$ 上的伪随机生成器，如果这两个分布 $D\_{0}=\\{\left(k, f\_{k}(x)\right) \mid k \leftarrow K, x \leftarrow X\\}$ 和 $D\_{1}=\\{(k, y) \mid k \leftarrow K, y \leftarrow Y\\}$ 是 $\epsilon$-不可区分的对于一些可忽略的 $\epsilon$，i.e.，对任意PPT算法 $\mathcal{A}\norm{a}$，*
